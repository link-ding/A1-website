...
const sitePages=JSON.parse(await readFile(path.join(root,'data/zh-site-pages.json'),'utf8'));
const managedContent=JSON.parse(await readFile(path.join(root,'data/default-content.json'),'utf8'));
...
function nav(currentSlug){
  return `<aside class="trial-banner" aria-label="试课信息"><div class="trial-banner-inner"><p>一对一试课按单次 1.5 小时课程收费；小班试课每 1.5 小时 $75。</p><a href="/contact/#trial-lesson">预约试课</a></div></aside><header class="site-header"><div class="header-inner"><a class="brand" href="/" aria-label="Academy One 首页"><img src="/assets/academy-one-crest.png" alt=""><span><strong>Academy One</strong><small>Private Tuition</small></span></a><nav class="primary-nav" aria-label="主导航"><a href="/courses">课程</a><a href="/zh/pricing/">学费</a><a href="/zh/tutors/">师资</a><a href="/timetable">课程表</a><a href="/careers/">招聘</a><a href="/contact/">联系</a><details class="nav-resources"><summary>资源</summary><div class="nav-resources-menu"><a href="/zh/resources/atar-calculator/">ATAR 估算工具</a><a href="/zh/resources/how-is-atar-calculated/">ATAR 如何计算</a><a href="/zh/resources/hsc-scaling/">HSC Scaling 详解</a><a href="/zh/resources/hsc-study-tips/">HSC 学习建议</a><a href="/zh/resources/selective-oc-guides/">精英中学 / OC 指南</a></div></details></nav><div class="header-actions"><a class="language-switch" href="/${currentSlug}/" data-language="en" lang="en-AU">English</a><a class="header-cta" href="${register}" target="_blank" rel="noopener">报名</a></div></div></header>`;
}

function footer(){
  return `<footer class="site-footer"><div class="footer-inner"><div class="footer-grid"><div><img class="footer-logo" src="/assets/academy-one-logo.png" alt="Academy One Private Tuition"><p>Suite 403, 815 Pacific Highway<br>Chatswood NSW 2067</p></div><div><h3>课程</h3><a href="/tutoring/maths/">数学辅导</a><a href="/tutoring/english/">英语辅导</a><a href="/zh/hsc-tutoring/">HSC 辅导</a><a href="/timetable">小班课程表</a><a href="/zh/pricing/">学费</a></div><div><h3>中心信息</h3><a href="/credit">请假、取消与课时抵扣政策</a><a href="/zh/resources/atar-calculator/">ATAR 估算工具</a><a href="/zh/resources/how-is-atar-calculated/">ATAR 如何计算</a><a href="/careers/">招聘</a><a href="/contact/">联系</a><a href="https://www.google.com/search?q=Academy+One+Private+Tuition+Chatswood" target="_blank" rel="noopener">Google 评价</a></div><div><h3>联系方式</h3><a href="tel:+61486017931">0486 017 931</a><p>微信：academyonechatswood</p><a href="mailto:operations@academyone.com.au">operations@academyone.com.au</a><a href="mailto:admin@academyone.com.au">admin@academyone.com.au</a></div></div><div class="copyright">© 2026 Academy One Private Tuition · Chatswood NSW</div></div></footer>`;
}
...
function standaloneHtml({slug,title,meta,h1,lede,body}){
  const english=`https://academyone.com.au/${slug}/`;
  const canonical=`https://academyone.com.au/zh/${slug}/`;
  const schema=[
    {'@context':'https://schema.org','@type':'EducationalOrganization',name:'Academy One Private Tuition',url:'https://academyone.com.au/',telephone:'+61486017931',email:'operations@academyone.com.au',address:{'@type':'PostalAddress',streetAddress:'Suite 403, 815 Pacific Highway',addressLocality:'Chatswood',addressRegion:'NSW',postalCode:'2067',addressCountry:'AU'}},
    {'@context':'https://schema.org','@type':'BreadcrumbList',itemListElement:[{'@type':'ListItem',position:1,name:'首页',item:'https://academyone.com.au/'},{'@type':'ListItem',position:2,name:title,item:canonical}]}
  ];
  return `<!doctype html><html lang="zh-Hans-AU"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)} | Academy One</title><meta name="description" content="${esc(meta)}"><link rel="canonical" href="${canonical}"><link rel="alternate" hreflang="en-AU" href="${english}"><link rel="alternate" hreflang="zh-Hans-AU" href="${canonical}"><link rel="alternate" hreflang="x-default" href="${english}"><meta property="og:type" content="website"><meta property="og:locale" content="zh_AU"><meta property="og:locale:alternate" content="en_AU"><meta property="og:title" content="${esc(title)} | Academy One"><meta property="og:description" content="${esc(meta)}"><meta property="og:url" content="${canonical}"><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Figtree:wght@400;500;600;700;800&family=Libre+Baskerville:wght@400;700&family=Noto+Sans+SC:wght@400;500;600;700&family=Noto+Serif+SC:wght@500;700&display=swap" rel="stylesheet"><link rel="stylesheet" href="/assets/seo-pages.css"><script type="application/ld+json">${JSON.stringify(schema)}</script></head><body>${nav(slug)}<main><div class="breadcrumbs"><a href="/">首页</a> <span aria-hidden="true">/</span> ${esc(title)}</div><section class="page-intro"><p class="eyebrow">Academy One · Chatswood</p><h1>${esc(h1)}</h1><p>${esc(lede)}</p></section>${body}</main>${footer()}<script src="/assets/seo-pages.js" defer></script></body></html>`;
}

function tutorDirectoryHtml(){
  const cards=managedContent.tutors.filter(tutor=>!tutor.hidden).map(tutor=>{
    const profile=tutor.zh;
    const displayName=firstName(profile.name);
    return `<article class="profile-card" id="${esc(tutor.id)}"><p class="eyebrow">${esc(profile.role)} · ${esc(profile.tag||tutor.group)}</p><h2>${esc(displayName)}</h2><p class="profile-subject"><strong>授课科目：</strong>${esc(profile.subject)}</p><p>${esc(profile.hook)}</p><details><summary>查看完整简介</summary><h3>学历与教学经验</h3><p>${esc(profile.qual)}</p><p>${esc(profile.bio)}</p><h3>部分教学成果</h3><p>${esc(profile.result)}</p><p><strong>授课形式与当前时间：</strong>请在咨询时向 Academy One 确认。</p></details><a class="text-link" href="/contact/">咨询 ${esc(displayName)} →</a></article>`;
  }).join('');
  return standaloneHtml({slug:'tutors',title:'Chatswood 与线上导师团队',meta:'查看 Academy One 英语、数学与科学导师的中文简介、授课科目、学历和教学经验。',h1:'认识 Academy One 导师团队。',lede:'浏览目前公开并经过确认的导师资料。具体课程匹配、授课形式与时间会在咨询时确认。',body:`<section class="directory section-wide">${cards}</section>`});
}

function pricingHtml(){
  const table=(heading,rows)=>`<section class="price-block"><h2>${heading}</h2><div class="price-scroll"><table><thead><tr><th>导师类别</th><th>Years 7–10<br>每小时</th><th>Years 11–12<br>每小时</th></tr></thead><tbody>${rows.map(row=>`<tr><th>${esc(row.labelZh)}</th>${row.values.slice(0,2).map(value=>`<td>${value==null?'暂不提供':`$${value}`}</td>`).join('')}</tr>`).join('')}</tbody></table></div></section>`;
  const group=managedContent.groupClasses;
  const body=`<div class="section-wide pricing-page">${table('英语辅导',managedContent.pricing.english)}${table('数学辅导',managedContent.pricing.maths)}${table('人文与科学辅导',managedContent.pricing.humanitiesScience)}<section class="price-block"><h2>小班课程 Package</h2><div class="group-prices"><p><strong>英语</strong><span>每节 1.5 小时约 $${group.english}<br>仅按完整 package 报名</span></p><p><strong>数学</strong><span>每节 1.5 小时约 $${group.mathematics}<br>仅按完整 package 报名</span></p></div><p class="group-note">每节课价格为折算后的约数。所有小班课程均按完整 package 出售，不提供单节购买。请<a href="/contact/">联系我们</a>确认当前课程表、package 周数和总费用。</p></section><p class="fine-print">所有一对一课程价格适用于整学期报名。缺课、退款和课时抵扣规则请查看<a href="/credit">相关政策</a>。</p></div>`;
  return standaloneHtml({slug:'pricing',title:'2027 辅导费用',meta:'Academy One 2027 年英语、数学、人文与科学一对一辅导费用及 Chatswood 小班 package 价格。',h1:'清晰了解 2027 年辅导费用。',lede:'整学期一对一课程根据导师类别与学生年级按小时收费；小班课程以完整 package 形式报名。',body});
}
...
function servicePageHtml(page){
  ...
  const faqJson=page.faq.map(([question,answer])=>({'@type':'Question',name:question,acceptedAnswer:{'@type':'Answer',text:answer}}));
  const schema=[
    {'@context':'https://schema.org','@type':'EducationalOrganization',name:'Academy One Private Tuition',url:'https://academyone.com.au/',telephone:'+61486017931',email:'operations@academyone.com.au'},
    {'@context':'https://schema.org','@type':'Service',name:page.title,inLanguage:'zh-Hans-AU',provider:{'@type':'EducationalOrganization',name:'Academy One Private Tuition'},areaServed:'Sydney NSW',url:canonical},
    {'@context':'https://schema.org','@type':'FAQPage',mainEntity:faqJson}
  ];
  return `<!doctype html><html lang="zh-Hans-AU"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(page.title)} | Academy One</title><meta name="description" content="${esc(page.meta)}"><link rel="canonical" href="${canonical}"><link rel="alternate" hreflang="en-AU" href="${english}"><link rel="alternate" hreflang="zh-Hans-AU" href="${canonical}"><link rel="alternate" hreflang="x-default" href="${english}"><meta property="og:type" content="website"><meta property="og:locale" content="zh_AU"><meta property="og:locale:alternate" content="en_AU"><meta property="og:title" content="${esc(page.title)} | Academy One"><meta property="og:description" content="${esc(page.meta)}"><meta property="og:url" content="${canonical}"><meta property="og:image" content="https://academyone.com.au/assets/academy-one-classroom.jpg"><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Figtree:wght@400;500;600;700;800&family=Libre+Baskerville:wght@400;700&family=Noto+Sans+SC:wght@400;500;600;700&family=Noto+Serif+SC:wght@500;700&display=swap" rel="stylesheet"><link rel="stylesheet" href="/assets/seo-pages.css"><script type="application/ld+json">${JSON.stringify(schema)}</script></head><body>${nav(page.slug)}<main><div class="breadcrumbs"><a href="/">首页</a> <span aria-hidden="true">/</span> ${esc(page.title)}</div><section class="hero"><img class="hero-image" src="/assets/academy-one-classroom.jpg" width="1672" height="941" alt="Academy One HSC 辅导课堂" fetchpriority="high"><div class="hero-inner"><div class="hero-copy"><p class="eyebrow">${esc(page.type)} · 悉尼</p><h1>${esc(page.h1)}</h1><p class="hero-lede">${esc(page.lede)}</p><div class="hero-actions"><a class="button" href="${register}" target="_blank" rel="noopener">提交报名意向</a><a class="text-link" href="/contact/">联系 Academy One →</a></div></div></div></section><section class="proof" aria-label="Academy One 简介"><div class="proof-inner"><div class="proof-item"><strong>98</strong><span>资深导师学生的 ATAR 中位数</span></div><div class="proof-item"><strong>2017</strong><span>自 2017 年开始辅导学生</span></div><div class="proof-item"><strong>Chatswood</strong><span>提供线下与线上课程</span></div></div></section><section class="section reveal"><div class="section-grid"><div><p class="eyebrow">我们的教学方式</p><h2>${esc(page.introTitle)}</h2></div><div class="prose">${page.intro.map(text=>`<p>${esc(text)}</p>`).join('')}</div></div></section><section class="section reveal"><div class="section-grid"><div><p class="eyebrow">课程范围</p><h2>根据学生的课程与下一阶段目标提供支持。</h2></div><div class="levels">${page.levels.map(([title,text])=>`<section class="level"><h3>${esc(title)}</h3><p>${esc(text)}</p></section>`).join('')}</div></div></section><section class="section reveal"><div class="section-grid"><div><p class="eyebrow">辅导流程</p><h2>用清晰的循环持续推进。</h2></div><ol class="topic-list">${page.topics.map(([title,text],index)=>`<li><span>0${index+1}</span><div><h3>${esc(title)}</h3><p>${esc(text)}</p></div></li>`).join('')}</ol></div></section><section class="section reveal"><div class="section-grid"><div><p class="eyebrow">常见问题</p><h2>咨询前可能想了解的问题。</h2></div><div class="faq-list">${page.faq.map(([question,answer])=>`<details><summary>${esc(question)}</summary><p>${esc(answer)}</p></details>`).join('')}</div></div></section><section class="related"><div class="related-inner"><p class="eyebrow">继续浏览</p><h2>为 HSC 学年建立更清晰的路径。</h2><div class="related-links">${page.related.map(([url,label])=>`<a href="${url}">${esc(label)} →</a>`).join('')}</div></div></section><section class="final-cta"><h2>告诉我们学生接下来需要什么。</h2><p>请提供学生年级、科目、目前遇到的困难和适合的上课时间。我们会协助推荐合适的导师和课程形式。</p><a class="button" href="/contact/">联系 Academy One</a></section></main>${footer()}<script src="/assets/seo-pages.js" defer></script></body></html>`;
}
...
function localizeCalculator(html){
  const replacements=[
    ...
    ['href="/hsc-tutoring/"','href="/zh/hsc-tutoring/"'],
    ['Academy One home','Academy One 首页'],
    ['ATAR calculator','ATAR 估算工具'],
    ['HSC tutoring','HSC 辅导']
  ];
...
for(const page of sitePages){
  const directory=path.join(root,'zh',page.slug);
  await mkdir(directory,{recursive:true});
  await writeFile(path.join(directory,'index.html'),servicePageHtml(page));
}

for(const [slug,html] of [['pricing',pricingHtml()],['tutors',tutorDirectoryHtml()]]){
  const directory=path.join(root,'zh',slug);
  await mkdir(directory,{recursive:true});
...
console.log(`Generated ${pages.length+sitePages.length+3} Chinese pages.`);
